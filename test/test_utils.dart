...
class MockQuranRepository implements IQuranRepository {
  ...
  Future<List<SurahModel>> getAllSurahs() async {
    return List.generate(114, (i) => SurahModel(
      number: i + 1,
  ...
  @override
  Future<AyahModel> getAyahBySurah(int surah, int ayahInSurah, [bool removeNewLines = true]) async {
    return AyahModel(
      id: 1,
      surah: surah,
  ...
  Future<List<JuzModel>> getJuzs() async {
    return List.generate(30, (i) => JuzModel(
      number: i + 1,
      glyph: 'Juz ${i + 1}',
    ));
  }
  ...
  Map<int, JuzModel> getJuzsSync() {
     return { for (var i=1; i<=30; i++) i: JuzModel(
      number: i,
      glyph: 'Juz $i',
    )};
  }
  ...
  @override
  JuzModel? getJuzSync(int number) => JuzModel(
      number: number,
      glyph: 'Juz $number',
    );
  ...
  Future<QuranPageModel> getPage(int page) async {
    ...
    if (page == 1) {
       ...
       blocks.add(SurahBlock(
         ...
         hasBasmalah: false,
         ayahs: [
           AyahFragment(ayahId: 10, start: 0, end: 5),
         ]
       ));
    }
